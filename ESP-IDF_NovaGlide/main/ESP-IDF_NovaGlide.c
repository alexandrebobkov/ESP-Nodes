...
void app_main(void)
{
    ...
    //esp_log_level_set("*", ESP_LOG_/ERROR);//_WARN);  // Global default
    ...
    for (;;) {
        uint8_t data[2];
        esp_err_t err = i2c_bus_read(ultra.dev, 0x02, data, 2);
        if (err == ESP_OK) {
            uint16_t raw_mm = ((uint16_t)data[0] << 8) | data[1];
            float cm = raw_mm / 10.0f;
            ESP_LOGI("ULTRA_TEST", "raw=%u mm, %.2f cm", raw_mm, cm);
        } else {
            ESP_LOGW("ULTRA_TEST", "read failed: %s", esp_err_to_name(err));
        }
        vTaskDelay(pdMS_TO_TICKS(1000));
    }
