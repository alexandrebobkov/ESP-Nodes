...
void sendData (void) {
    ...
    ESP_LOGI(TAG, "pcm 1: 0x%04X, pcm 2: 0x%04X", (uint8_t)buffer.motor1_rpm_pcm, (uint8_t)buffer.motor2_rpm_pcm);
    ESP_LOGI(TAG, "pcm 3: 0x%04X, pcm 4: 0x%04X", (uint8_t)buffer.motor3_rpm_pcm, (uint8_t)buffer.motor4_rpm_pcm);
