        ...
        */
        ...
        if (gpio_set_args.gpio->count > 0 && gpio_set_args.level->count > 0 && gpio_set_args.mode->count > 0) {
            ...
            if (strcmp(gpio_set_args.mode->sval[0], "out") == 0) {
                ...
                ESP_LOGW("gpio-set", "GPIO %d level is %d", gpio_set_args.gpio->ival[0], gpio_get_level(gpio_set_args.gpio->ival[0]));
            }
            if (strcpy(gpio_set_args.mode->sval[0], "reset") == 0) {
...
static void register_gpio_set_cmd (void) {
    ...
    gpio_set_args.mode  = arg_str0("m", "mode",     "<reset|in|out>", "Sets the mode of GPIO.");
